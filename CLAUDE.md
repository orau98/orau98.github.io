...
- トップページは検索ファーストとし、検索ヒーローの中（検索窓の横）に昆虫／植物の切り替えタブを置き、ヒーローの直後に結果を置く。
- 既存の検索・タブ・フィルターと目的が重なるカードや入口を、検索と結果の間へ追加しない。
- `public/meta/` と `public/en/meta/` はSEO・検索流入・サイトマップ向けの静的ページであり、通常の主要導線へ昇格させない。
- 新しいショートカットが必要な場合は、静的な文字一覧ではなくSPAの検索・フィルター状態へ接続し、戻る操作と状態を保つ。
- ホームの導線を追加・変更する前に、既存機能との重複、デスクトップとモバイルの視覚階層、主結果が初期画面から押し出されないことを確認する。

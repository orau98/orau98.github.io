...
日本産の蛾・蝶・タマムシ・カミキリムシ・キクイムシ・ハムシ・アブラムシと食草の関係を収録した昆虫植物図鑑です。
...
- 7分類群を収録（蛾・蝶・タマムシ・カミキリムシ・キクイムシ・ハムシ・アブラムシ）
...
### 『日本のハマキガ1』全種監査（2026-07-12）

- 原PDFの全216種アカウントを見出し単位で照合し、現行216分類群へ一意に対応付けました。
- 寄主植物欄の内訳は、国内寄主あり129種、原典で寄主不明63種、国内寄主は不明だが国外寄主あり24種です。
- 監査結果として寄主52行を追加（国外51、国内1）、意味重複3行を削除、既存フィールド5件を修正しました。一般注記は19件を修正、推論だけの1件を削除し、出現時期1件を追加しました。
- 新規寄主行の公開 `notes` は空欄とし、国外の地域・原著植物学名・PDFページは監査台帳の `evidence` にのみ保持しています。
- 途中で切れていた生態注記は客観的な摂食記述までを復元しました。さらに原PDFで13件のOCR段組み空白を修正し、主観的な美観評価1件と推論・推測表現6件を観察事実から文単位で分離しました。全文が推論だけの成虫越冬注記1件は削除し、原典の樹脂回避仮説も公開対象外としています。
- 全件台帳: `data/source_audits/japanese-tortricid-moths-1-all-accounts-2026-07-12.csv`
- 適用差分台帳: `data/source_audits/japanese-tortricid-moths-1-integrity-2026-07-12.csv`
- 再適用可能な適用コマンド: `npm run apply:hamakiga1-literature-audit`

### 標準図鑑1〜3の一般注記監査（2026-07-12）

- 全文献注記の品質検査から抽出した48行を、3冊の原PDF画像へ戻って確認しました。
- 第1巻は29行を修正し、推論だけ／「生態未詳」だけの2行を削除しました。第2・3巻は17行を修正しました。
- 原PDFと台帳のハッシュ、対象ID、変更前後の本文・ページを固定し、部分適用や改変を拒否する再適用可能な処理にしています。

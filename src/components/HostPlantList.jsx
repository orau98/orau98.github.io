  ...
  <li className="group relative overflow-hidden rounded-xl bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm border border-white/20 dark:border-slate-700/50 hover:border-teal-300 dark:hover:border-teal-500 transition-all duration-300 hover:shadow-lg hover:shadow-teal-500/20 hover:scale-[1.02] transform">
    <Link to={`/plant/${encodeURIComponent(plant)}`} className="block p-5">
      <div className="flex items-start space-x-3">
        <div className="flex-shrink-0 w-3 h-3 bg-gradient-to-r from-teal-400 to-emerald-400 rounded-full mt-1.5 group-hover:scale-110 transition-transform"></div>
...
const HostPlantList = ({ hostPlants, plantDetails }) => {
  ...
  return (
    <div className="bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl rounded-2xl shadow-xl border border-white/20 dark:border-slate-700/50 overflow-hidden">
      ...
      <div className="p-6">
        <ul className="space-y-3 max-h-96 overflow-y-auto scrollbar-thin scrollbar-thumb-teal-300 scrollbar-track-teal-100 dark:scrollbar-thumb-teal-600 dark:scrollbar-track-teal-900/20">
          {currentHostPlants.length > 0 ? (
            currentHostPlants.map(([plant, mothList], index) => (
              <div key={plant} className="animate-fadeIn" style={{ animationDelay: `${index * 0.05}s` }}>
                <HostPlantListItem plant={plant} mothCount={mothList.length} />
              </div>
            ))
          ) : (

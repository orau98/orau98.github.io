// DEV専用のレガシーCSVデータパイプライン。
// 正規化CSV群（insects.csv / hostplants.csv / general_notes.csv）や
// 旧来の各種CSV（ListMJ / YList / 和名チェックリスト / ハムシ統合 等）を
// クライアントで取得・解析してデータセットを構築する。
//
// 本番ビルドは assets/data-lite/ のJSONで完結し、この経路には入らない
// （App.jsx 側で import.meta.env.PROD なら早期return）。そのため本モジュールは
// App.jsx から動的importされ、メインバンドルには含まれない。
import logger from '../utils/logger';
import { saveDatasetToCache } from './datasetCache';
...
export async function runLegacyCsvPipeline(ctx) {
  const {
    shouldContinue,
    externalSignal,
    allowDebugLogs,
      ...
      try {
        ...
        beetleParsed.data.forEach((row, index) => {
          ...
          const beetle = {
            ...
            type: 'beetle',
            alternativeNames: collectAlternativeNames(row, { dedupe: true, excludePrimary: japaneseName }),
            classification: {
              family: family,
        ...
        if (!useNormalizedOnly && hamushiParsedData.length > 0) {
          ...
          const leafbeetle = {
            ...
            type: 'leafbeetle',
            alternativeNames: collectAlternativeNames(row),
            classification: {

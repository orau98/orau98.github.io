...
const DATA_PATH = path.join('data', 'moth_ecology_book2_batch1.csv');
...
const REPORT_PATH = path.join('reports', 'book2_batch1_notes_report.json');
...
function main() {
  ...
  for (const row of batchRows) {
    ...
    if (emergenceRaw && !shouldSkip(emergenceRaw)) {
      ...
      if (!existing.has(key)) {
        const recordId = `note-B2B1-E-${timestamp}-${String(emergenceCounter++).padStart(3, '0')}`;
        additions.push({
          record_id: recordId,
          insect_id: insectId,
          note_type: '出現時期',
    ...
    if (ecologyRaw && !shouldSkip(ecologyRaw)) {
      ...
      if (!existing.has(key)) {
        const recordId = `note-B2B1-Eco-${timestamp}-${String(ecologyCounter++).padStart(3, '0')}`;
        additions.push({
          record_id: recordId,
          insect_id: insectId,
          note_type: '生態情報',

...
// resized 画像から AVIF/WebP/JPG のレスポンシブ <picture> を生成する。
...
function buildResponsivePicture({ dir, base, alt, aboveFold = false }) {
  ...
  const srcset = (fmt) => widths.map((w) => `/images/resized/${dir}/${enc}.${w}.${fmt} ${w}w`).join(', ');
...
function buildHeroPreloadLink({ dir, base }) {
  ...
  const dirAbs = path.join(PUBLIC_DIR_ABS, 'images/resized', dir);
  const widths = RESIZED_HERO_WIDTHS.filter((w) =>
    fs.existsSync(path.join(dirAbs, `${decodedBase}.${w}.avif`)),
  );
  if (!widths.length) return '';
  const enc = encodeURIComponent(decodedBase);
  const maxW = Math.max(...widths);
  const imagesrcset = widths.map((w) => `/images/resized/${dir}/${enc}.${w}.avif ${w}w`).join(', ');
  return `<link rel="preload" as="image" type="image/avif" fetchpriority="high" href="/images/resized/${dir}/${enc}.${maxW}.avif" imagesrcset="${imagesrcset}" imagesizes="(max-width: 640px) 100vw, 640px">`;
}

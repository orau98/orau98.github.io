...
const NOTES_PATH = resolvePath('JMSG1_FULL_SCAN_NOTES_PATH', 'normalized_data/general_notes.csv');
const AUDIT_PATH = resolvePath(
  'JMSG1_FULL_SCAN_AUDIT_PATH',
  'data/source_audits/japanese-moths-standard-guide-1-full-scan-note-integrity-2026-07-12.json',
);
const PDF_PATH = process.env.JMSG1_FULL_SCAN_PDF_PATH
  ? path.resolve(process.env.JMSG1_FULL_SCAN_PDF_PATH)
  : null;
...
const EXPECTED_PRODUCTION_LEDGER_SHA256 = '2819fb268b12d08db8d46a1af4d464d40279f2121a90809c807e852c4dbc5355';
const EXPECTED_SOURCE_PDF_SHA256 = '35e3a46b1c91a6f16a6aff7522be708ed8a94ccfc8dc39d23c3d3e9171cfd0d9';
const EXPECTED_NOTE_TYPES = new Set(['生態情報', '出現時期']);
...
if (audit.audit_version !== '2026-07-12-full-scan') throw new Error('Unexpected audit_version');
if (audit.reference !== '日本産蛾類標準図鑑1') throw new Error('Unexpected reference');
if (
  !Array.isArray(audit.note_types)
  || audit.note_types.length !== EXPECTED_NOTE_TYPES.size
  || audit.note_types.some((noteType) => !EXPECTED_NOTE_TYPES.has(noteType))
) {
  throw new Error('Unexpected note_types');
}
...
if (
  audit.scope?.reviewed_reference_rows !== 1485
  || audit.scope?.prior_ledger_entries_excluded !== 31
  || audit.scope?.candidate_entries !== expectedTotal
  || audit.scope?.expected_entries !== expectedTotal
  || audit.entries.length !== expectedTotal
) {
  throw new Error('Unexpected full-scan scope or entry count');
}
...
if (
  !Array.isArray(audit.scope?.irrecoverable_entries)
  || audit.scope.irrecoverable_entries.length !== 1
  || audit.scope.irrecoverable_entries[0]?.record_id !== 'note-1001307'
  || !audit.scope.irrecoverable_entries[0]?.reason
  || !Array.isArray(audit.scope?.hold_entries)
  || audit.scope.hold_entries.length !== 0
) {
  throw new Error('Unexpected irrecoverable or hold entries');
}
...
for (const entry of audit.entries) {
  ...
  if (!EXPECTED_NOTE_TYPES.has(entry.note_type)) {
    throw new Error(`Unsupported note_type for ${entry.audit_id}: ${entry.note_type}`);
  }
  ...
  if (!Array.isArray(entry.candidate_reasons) || entry.candidate_reasons.length === 0) {
    throw new Error(`Missing candidate reasons for ${entry.audit_id}`);
  }
  ...
  if (entry.action === 'replace') {
    ...
    if (entry.approved_content !== '' || entry.approved_page !== '') {
      throw new Error(`${entry.action} entry must not contain approved data for ${entry.audit_id}`);
    }
    if (
      entry.action === 'delete'
      && (
        (entry.intermediate_content === undefined) !== (entry.intermediate_page === undefined)
        || entry.intermediate_content === entry.expected_content
      )
    ) {
      throw new Error(`Invalid intermediate delete state for ${entry.audit_id}`);
    }
  }
...
const enforceProductionAudit = path.resolve(AUDIT_PATH) === path.resolve(PRODUCTION_AUDIT_PATH)
  || process.env.JMSG1_FULL_SCAN_ENFORCE_PRODUCTION === '1';
...
for (const entry of audit.entries) {
  ...
  if (entry.action === 'delete') {
    const isExpectedState = currentContent === entry.expected_content
      && currentPage === entry.expected_page;
    const isIntermediateState = entry.intermediate_content !== undefined
      && currentContent === entry.intermediate_content
      && currentPage === entry.intermediate_page;
    if (!isExpectedState && !isIntermediateState) {
...
for (const entry of audit.entries) {
  ...
  if (entry.action === 'hold') {
    if (
      ...
      || row.content !== entry.expected_content
      || row.page !== entry.expected_page
    ) {
      throw new Error(`Held note is not exact after transformation: ${entry.record_id}`);
    }
    continue;
  }
  if (
...
console.log(JSON.stringify({
  reviewed_reference_rows: audit.scope.reviewed_reference_rows,
